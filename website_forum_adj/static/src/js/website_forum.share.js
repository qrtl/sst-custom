odoo.define("website_forum.share", function(require) {
  ...
  var ForumShare = sAnimation.registry.socialShare.extend({
    // Modified by QTL
    xmlDependencies: sAnimation.registry.socialShare.prototype.xmlDependencies.concat([
      "/website_forum_adj/static/src/xml/website_forum_share_templates.xml",
    ]),
    read_events: {},
    ...
    _render: function() {
      if (!this.targetType) {
        ...
        $(".row .question").before(
          qweb.render("website.social_alert", {medias: this.socialList})
        );
      } else {
        $("body").append(
  ...
  base.ready().then(function() {
    ...
    $(document.body).on(
      "click",
      ":not(.karma_required).oe_social_share_call",
      function() {
