    ...
    def web_login(self, redirect=None, **kw):
        ...
        if request.httprequest.method == 'POST':
            ...
            if uid is not False:
                    ...
                    if multi_ok:
                        ...
                        if user.login_calendar_id:
                            ...
                            if attendances:
                                ...
                                unsuccessful_message = \
                                    "unsuccessful login from '%s', user time "\
                                    "out of allowed calendar defined in user"\
                            ...
                            for group in user.groups_id:
                                ...
                                if sessions and group.multiple_sessions_block\
                                    ...
                                    unsuccessful_message = _(
                                        "unsuccessful login from '%s', "
                                        "multisessions block defined in group "
                                        "'%s'") % (
                                        request.params['login'], group.name)
                                    break
                            if calendar_set > 0 and calendar_ok is False:
                                unsuccessful_message = _(
                                    "unsuccessful login from '%s', user time "
                                    "out of allowed calendar defined in group "
                                    "'%s'") % (
                ...
                unsuccessful_message = _("unsuccessful login from '%s', wrong "
            ...
            request.uid = old_uid
            values['error'] = \
                _('Login failed due to one of the following reasons:')
            values['reason1'] = \
                _('- Wrong login/password')
            values['reason2'] = _('- User not allowed to have multiple logins')
            values[
                'reason3'] = \
                _('- User not allowed to login at this specific time or day')
...
class Session_tkobr(Session):
    ...
    def logout(self, redirect='/web'):
        ...
        return super(Session_tkobr, self).logout(redirect=redirect)
