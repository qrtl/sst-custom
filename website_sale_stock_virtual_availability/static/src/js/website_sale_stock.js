odoo.define("website_sale_stock_virtual_availability.website_sale", function(require) {
  ...
  var xml_load = ajax.loadXML(
    "/website_sale_stock_virtual_availability/static/src/xml/website_sale_stock_product_availability.xml",
    QWeb
  );
  ...
  $(".oe_website_sale").each(function() {
    ...
    $(oe_website_sale).on("change", 'input[name="add_qty"]', function() {
      $(oe_website_sale)
        .find("ul[data-attribute_value_ids]")
        .trigger("change");
    });
      ...
      for (var k in variant_ids) {
        if (_.isEmpty(_.difference(variant_ids[k][1], values))) {
          ...
          if (_.contains(["always", "threshold"], info.inventory_availability)) {
            ...
            // info['website_sale_available_qty'] -= parseInt(info['cart_qty']);
            // << QTL ADD
            if (info.website_sale_available_qty < 0) {
              info.website_sale_available_qty = 0;
            }
            // Handle case when manually write in input
            if (qty > info.website_sale_available_qty) {
              $parent
                .find('input[name="add_qty"]')
                .val(info.website_sale_available_qty || 1);
            }
            if (
              qty > info.website_sale_available_qty ||
              info.website_sale_available_qty < 1 ||
              qty < 1
            ) {
            ...
            $parent
              .find(".js_add")
              .toggleClass("disabled btn", info.website_sale_available_qty < 1);
          }
          xml_load.then(function() {
            $(oe_website_sale)
              .find(".availability_message_" + info.product_template)
              .remove();
            var $message = $(
              QWeb.render("website_sale_stock.product_availability", info)
            );
