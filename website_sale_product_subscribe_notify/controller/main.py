...
class CustomerPortal(CustomerPortal):
    def _prepare_portal_layout_values(self):
        ...
        values.update({"product_subscription_count": subscribe_count})
        return values

    @http.route(
        ["/my/product_subscriptions", "/my/product_subscriptions/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def portal_product_subscriptions(self, **kw):
        ...
        product_public_category_lst = (
            product_public_category.search([])
            .filtered(
                lambda categ: request.env.user.partner_id.id
                in categ.message_partner_ids.ids
            )
            .ids
        )
        if "error" in kw:
            values.update({"error": kw.get("error")})
        values.update(
            {
    ...
    def product_subscribes(self, **kw):
        ...
        selected_category_list = (
            request.env["product.public.category"].sudo().browse(category_list)
        )
        ...
        if (
            partner.member_group_id
            and partner.member_group_id.point_limit < subscribe_point_count
            or not partner.member_group_id
        ):
            return self.portal_product_subscriptions(
                error=_(
                    "You subscription point is not enough to subscribe "
        ...
        return request.render(
            "website_sale_product_subscribe_notify.template_thanks_message", vals
        )
