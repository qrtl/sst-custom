...
class OdooLocation(object):
    def __init__(
        self, location, db_name, username, password, use_custom_api_path=False
    ):
    ...
    def call(self, model, method, arguments):
        ...
        try:
            ...
            try:
                ...
                if method == "search":
                    # result = odoo.env[method].search(
                    # [('write_date', '>=', '2018-08-01 00:00:00'),
                    # ('write_date', '<=', '2018-08-13 23:59:59')])
                    odoo_field = next(iter(arguments[0]))
                    # result = odoo.env[method].search(
                    # [(odoo_field, '>=', '2018-08-01 00:00:00'),
                    # (odoo_field, '<=', '2018-08-13 23:59:59')])
                    result = odoo.env[model].search(
                        [
                            (odoo_field, ">=", arguments[0][odoo_field]["from"]),
                            (odoo_field, "<=", arguments[0][odoo_field]["to"]),
                        ]
                    )
                if method == "read":
...
class OdooCRUDAdapter(AbstractComponent):
    ...
    def _call(self, model, method, arguments):
        try:
            odoo_api = getattr(self.work, "odoo_api", None)
...
class GenericAdapter(AbstractComponent):
    ...
    def read(self, id, attributes=None):
        ...
        return self._call("%s.info" % self._magento_model, arguments)
    ...
    def write(self, id, data):
        """ Update records on the external system """
        return self._call("%s.update" % self._magento_model, [int(id), data])
    ...
    def admin_url(self, id):
        ...
        path = self._admin_path.format(model=self._magento_model, id=id)
        url = url.rstrip("/")
        path = path.lstrip("/")
        url = "/".join((url, path))
        return url
