...
class AccountInvoice(models.Model):
    ...
    total_delivered_amount = fields.Monetary(
        string="Total Delivered Amount",
        currency_field="currency_id",
        store=True,
        compute="_compute_delivered_amount",
    )
    total_delivered_amount_signed = fields.Monetary(
    ...
    def _compute_delivered_amount(self):
        for invoice in self:
            invoice.total_delivered_amount = sum(
                line.deliverd_amount for line in invoice.invoice_line_ids
            )
            ...
            if (
                ...
                total_delivered_amount_signed = currency_id.compute(
                    invoice.total_delivered_amount, invoice.company_id.currency_id
                )
