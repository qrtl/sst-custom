    ...
    def cart_update_json(
        ...
        if line_id and set_qty and not add_qty:
            ...
            penalty_product_id = (
                ...
                .sudo()
                .get_param("website_sale_remove_product_penalty.penalty_product_id")
            )
            # Since this method will only be called on frontend, no need to
            # check whether it is a "Website Sales" order or not.
            if (
                sale_order_line.product_uom_qty > set_qty
                and penalty_product_id
                and sale_order_line.product_id.type == "product"
            ):
                penalty_product = (
                    request.env["product.product"]
                    .sudo()
                    .browse([int(penalty_product_id)])[0]
                )
                ...
                sale_order.sudo().write(
                    {
                        "cart_product_removal_history_ids": [
                            (
                                0,
                                0,
                                {
