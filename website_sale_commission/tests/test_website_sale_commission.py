...
class WebsiteSaleCommission(common.TransactionCase):
    def setUp(self):
        ...
        self.website_sale_order = (
            self.env["sale.order"]
            .sudo(self.test_user.id)
            .create(
                {
        ...
        self.website_commission_product_id = self.env["product.product"].create(
            {"name": "Commission", "type": "service", "list_price": 100}
        )
    ...
    def test_00_add_product_website_order_comission_percentage(self):
        ...
        commission_line = self.website_sale_order.order_line.filtered(
            lambda i: i.is_commission
        )
        self.assertTrue(commission_line)
        self.assertEqual(
            commission_line.price_unit, product_order_line.price_unit * 10.0 / 100
        )
    ...
    def test_01_add_product_website_order_fix_comission(self):
        ...
        self.assertTrue(commission_line)
        self.assertEqual(
            commission_line.price_unit, self.website_commission_product_id.list_price
        )
    ...
    def test_02_get_display_price(self):
        ...
        self.assertEqual(
            commission_line._get_display_price(self.website_commission_product_id),
