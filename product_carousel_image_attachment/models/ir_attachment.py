...
class IrAttachment(models.Model):
    ...
    def create(self, vals):
        ...
        if mimetype in IMAGE_TYPES and "datas" in vals:
            ...
            datas = (
                vals["datas"].encode("utf8")
                if type(vals["datas"]) is str
                else vals["datas"]
            )
            vals["datas"] = image_resize_image(
                datas,
                size=(1024, 1024),
                encoding="base64",
                filetype=None,
        ...
        if (
            attachment
            and attachment.mimetype in IMAGE_TYPES
            and attachment.res_model in ["product.template", "product.product"]
            and attachment.datas_fname
        ):
            # assignment for pt
